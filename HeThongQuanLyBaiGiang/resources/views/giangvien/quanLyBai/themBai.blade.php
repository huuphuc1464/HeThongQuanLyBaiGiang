...
@section('title','Giảng viên - Thêm bài học')
@section('tenTrang', $baiHoc->TenBaiGiang . ' / ' . $baiHoc->TenChuong . ' / Thêm bài')
@section('content')
<div class="row">
    <div class="col-md-12">
        <div class="tile">
            <h3 class="tile-title">Thêm bài học</h3>
            <div class="tile-body">
                <form action="{{ route('giangvien.bai-giang.chuong.bai.them', ['maBaiGiang' => $baiHoc->MaBaiGiang, 'maChuong' => $baiHoc->MaChuong]) }}" method="POST" class="row" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" name="TenBai" class="form-control" required value="{{ old('TenBai') }}">
                        @error('TenBai')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" name="MoTa" class="form-control" value="{{ old('MoTa') }}">
                        @error('MoTa')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-12">
                        <label for="MoTa" class="mb-2">Nội dung bài giảng <span class="text-danger">*</span></label>
                        @error('NoiDung')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                        <textarea name="NoiDung" id="editor" class="form-control">{{ old('NoiDung') }}</textarea>
                    </div>
