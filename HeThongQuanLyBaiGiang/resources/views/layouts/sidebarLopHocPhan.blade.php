<nav class="sidebar d-flex flex-column" id="sidebar" style="height: calc(100vh - 64px);">
    <div class="flex-shrink-0">
        ...
        <div class="duongKe section-title ps-3 nav-link active d-flex align-items-center justify-content-between m-0">
            <div class="d-flex align-items-center gap-2">
                <img src=" {{ $lopHocPhan->AnhBaiGiang ? asset( $lopHocPhan->AnhBaiGiang) : asset('img/hocphan/default.png') }}"
                    width="30" height="32" alt="Class icon" style="border-radius: 50%; border: 1.5px solid #3a3a3a;" />
                <span class="text-truncate d-inline-block"
                    style="max-width: 195px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;"
    ...
    <div style="flex-grow: 1; overflow-y: auto;">
        <div class="accordion" id="sidebarAccordion">
            @foreach ($danhSachBaiGiangSidebar as $tenChuong => $dsBaiGiang)
            @php $chuongId = Str::slug($tenChuong) . '-' . uniqid(); @endphp
            <div class="accordion-item border-0">
                <h2 class="accordion-header" id="heading-{{ $chuongId }}">
                    <button class="accordion-button collapsed bg-white text-dark px-3 fw-bold" style="padding: 0.75rem"
                        type="button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $chuongId }}"
                        aria-expanded="false" aria-controls="collapse-{{ $chuongId }}">
