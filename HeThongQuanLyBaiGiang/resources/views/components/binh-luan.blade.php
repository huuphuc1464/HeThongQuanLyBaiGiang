    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('binhluan.guibinhluan') }}">
                @csrf
                <input type="hidden" name="MaBaiGiang" value="{{ $baiGiang->MaBaiGiang }}">
                <div class="d-flex mb-3">
                    <textarea name="NoiDung" class="form-control me-2" rows="2" placeholder="Nhập bình luận mới..." required>{{ old('NoiDung') }}</textarea>
                    <button type="submit" class="btn btn-success" style="width: 40px; height: 40px;"><i class="fas fa-paper-plane"></i></button>
                </div>
                @error('NoiDung')
                ...
                <div class="d-flex align-items-start">
                    ...
                    <div class="w-100">
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            {{-- Dropdown 3 chấm --}}
                            <div class="dropdown">
                                <button class="btn btn-sm p-0 text-secondary border-0 bg-transparent" type="button" data-bs-toggle="dropdown">
                                    <i class="fas fa-ellipsis-v"></i>
                                </button>
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        <a class="dropdown-item text-primary" href="javascript:void(0);" onclick="hienFormSua({{ $binhLuan->MaBinhLuan }})">
                                    ...
                                    <li>
                                        <form method="POST" action="{{ route('binhluan.xoa', $binhLuan->MaBinhLuan) }}">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="dropdown-item text-danger" onclick="return confirm('Xác nhận xóa bình luận?')">
                        ...
                        {{-- Form sửa --}}
                        <form method="POST" action="{{ route('binhluan.capnhat') }}" class="d-none" id="form-sua-{{ $binhLuan->MaBinhLuan }}">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="MaBinhLuan" value="{{ $binhLuan->MaBinhLuan }}">
                            <textarea name="NoiDung" class="form-control mb-2" rows="2" required>{{ $binhLuan->NoiDung }}</textarea>
                            <div class="d-flex justify-content-end gap-2">
                                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="anFormSua({{ $binhLuan->MaBinhLuan }})">Hủy</button>
                                <button type="submit" class="btn btn-sm btn-primary">Lưu</button>
                            </div>
                        </form>

                        <div class="mb-2">
                            <a href="javascript:void(0);" class="text-primary text-decoration-none" onclick="hienFormTraLoi({{ $binhLuan->MaBinhLuan }})">
                                <i class="fas fa-share fa-rotate-270 fa-flip-vertical"></i> Phản hồi
                            </a>
                        </div>

                        {{-- Form trả lời --}}
                        <form method="POST" action="{{ route('binhluan.traloi') }}" class="ms-4 mt-2 d-none" id="form-tra-loi-{{ $binhLuan->MaBinhLuan }}">
                            @csrf
                            <input type="hidden" name="MaBinhLuan" value="{{ $binhLuan->MaBinhLuan }}">
                            <input type="hidden" name="MaBaiGiang" value="{{ $baiGiang->MaBaiGiang }}">
                            <textarea name="NoiDung" class="form-control mb-2" rows="2" placeholder="Trả lời..." required></textarea>
                            <div class="d-flex justify-content-end gap-2">
                                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="anFormTraLoi({{ $binhLuan->MaBinhLuan }})">Hủy</button>
                        ...
                        <a href="javascript:void(0);" class="text-decoration-none ms-4" onclick="toggleTraLoi({{ $binhLuan->MaBinhLuan }})">
